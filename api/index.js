...
const app = express();
const DATA_FILE = process.env.DATA_FILE || path.join('/tmp', 'user-data.json');
...
app.post('/api/data/:collection', (req, res) => {
  ...
  data[collection].push({ ...req.body, id: Date.now().toString() + Math.random().toString(36).slice(2), createdAt: new Date().toISOString() });
  saveData(data);
